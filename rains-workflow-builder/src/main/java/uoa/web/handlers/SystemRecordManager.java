	...
	public void addSystemInfo(NewSystemForm newSystem) {
		...
		/*	
			...
			*/
			...
			Resource installationPlan = f.createIRI(Constants.DEFAULT_INSTANCE_NAMESPACE+UUID.randomUUID());
			conn.add(installationPlan, RDF.TYPE, f.createIRI(RainsOntologyComponents.InstallationAccountabilityPlan), planNamedGraphContext);
			conn.add(installationPlan, RDF.TYPE, f.createIRI(RainsOntologyComponents.AccountabilityPlan), planNamedGraphContext);
			conn.add(installationPlan, RDF.TYPE, f.createIRI(OwlOntologyComponents.NamedIndividual), planNamedGraphContext);
			conn.add(installationPlan, f.createIRI(RainsOntologyComponents.specifiedForSystem),system, planNamedGraphContext);
			
			Resource userTrainingPlan = f.createIRI(Constants.DEFAULT_INSTANCE_NAMESPACE+UUID.randomUUID());
			conn.add(userTrainingPlan, RDF.TYPE, f.createIRI(RainsOntologyComponents.UserTrainingAccountabilityPlan), planNamedGraphContext);
			conn.add(userTrainingPlan, RDF.TYPE, f.createIRI(RainsOntologyComponents.AccountabilityPlan), planNamedGraphContext);
			conn.add(userTrainingPlan, RDF.TYPE, f.createIRI(OwlOntologyComponents.NamedIndividual), planNamedGraphContext);
			conn.add(userTrainingPlan, f.createIRI(RainsOntologyComponents.specifiedForSystem),system, planNamedGraphContext);
			
			Resource manualCreationPlan = f.createIRI(Constants.DEFAULT_INSTANCE_NAMESPACE+UUID.randomUUID());
			conn.add(manualCreationPlan, RDF.TYPE, f.createIRI(RainsOntologyComponents.ManualCreationStageAccountabilityPlan), planNamedGraphContext);
			conn.add(manualCreationPlan, RDF.TYPE, f.createIRI(RainsOntologyComponents.AccountabilityPlan), planNamedGraphContext);
...
HashMap <String,String > map = new  HashMap <String,String >  ();
		...
		case "deploymentInstallation":
			stageIRI = RainsOntologyComponents.InstallationAccountabilityPlan;
		break; 
		
		case "deploymentUserTraining":
			stageIRI = RainsOntologyComponents.UserTrainingAccountabilityPlan;
		break;
		
		case "deploymentManual":
  ...
  ArrayList <HashMap <String,String >> list = new  ArrayList  <HashMap <String,String >>   ();
	...
	public String  getPlanElementsAsGraph(String systemIri, String stage) {
		  ...
		  if (stage.toLowerCase().equals("deploymentinstallation")) {
			  planType = RainsOntologyComponents.InstallationAccountabilityPlan;
		  }
		  
		  if (stage.toLowerCase().equals("deploymentusertraining")) {
			  planType = RainsOntologyComponents.UserTrainingAccountabilityPlan;
		  }
		  
		  if (stage.toLowerCase().equals("deploymentmanual")) {
	...
	public ArrayList <HashMap>  getPlanElementsForRuntimeAccountabilityPlan(String systemIri) {
		  ...
		  ArrayList <HashMap> resultSet = new ArrayList <HashMap> ();
			String queryString = Constants.PREFIXES + "Select Distinct ?element ?elementType ?input ?inputType ?output ?outputType ?row FROM <"+getPlansNamedGraph(systemIri)+"> WHERE {?plan rdf:type rains:RuntimeAccountabilityPlan; <"+RainsOntologyComponents.specifiedForSystem+"> <"+systemIri+">. ?element ep-plan:isElementOfPlan ?plan.  ?element a ?elementType. OPTIONAL {?element ep-plan:hasInputVariable ?input. ?element af:belongsToRow ?row. ?input a ?inputType. FILTER (regex(str(?inputType), \"https://w3id.org/rains#\" ))} OPTIONAL {?element ep-plan:hasOutputVariable ?output. ?output a ?outputType. FILTER (regex(str(?outputType), \"https://w3id.org/rains#\" ))} FILTER (regex(str(?elementType), \"https://w3id.org/rains#\" ))}";
	...
	public ArrayList <HashMap>  getPlanElementsForConfigurationAccountabilityPlan(String systemIri) {
		...
		ArrayList <HashMap> resultSet = new ArrayList <HashMap> ();
			String queryString = Constants.PREFIXES + "Select Distinct ?element ?elementType ?input ?inputType ?output ?outputType ?row FROM <"+getPlansNamedGraph(systemIri)+"> WHERE {?plan rdf:type rains:ConfigurationAccountabilityPlan; <"+RainsOntologyComponents.specifiedForSystem+"> <"+systemIri+">. ?element ep-plan:isElementOfPlan ?plan.  ?element a ?elementType. OPTIONAL {?element ep-plan:hasInputVariable ?input. ?element af:belongsToRow ?row. ?input a ?inputType. FILTER (regex(str(?inputType), \"https://w3id.org/rains#\" ))} OPTIONAL {?element ep-plan:hasOutputVariable ?output. ?output a ?outputType. FILTER (regex(str(?outputType), \"https://w3id.org/rains#\" ))} FILTER (regex(str(?elementType), \"https://w3id.org/rains#\" ))}";
	...
	public ArrayList <HashMap>  getPlanElementsForMaintenanceAccountabilityPlan(String systemIri) {
		...
		ArrayList <HashMap> resultSet = new ArrayList <HashMap> ();
			String queryString = Constants.PREFIXES + "Select Distinct ?element ?elementType ?input ?inputType ?output ?outputType ?row FROM <"+getPlansNamedGraph(systemIri)+"> WHERE {?plan rdf:type rains:MaintenanceAccountabilityPlan; <"+RainsOntologyComponents.specifiedForSystem+"> <"+systemIri+">. ?element ep-plan:isElementOfPlan ?plan.  ?element a ?elementType. OPTIONAL {?element ep-plan:hasInputVariable ?input. ?element af:belongsToRow ?row. ?input a ?inputType. FILTER (regex(str(?inputType), \"https://w3id.org/rains#\" ))} OPTIONAL {?element ep-plan:hasOutputVariable ?output. ?output a ?outputType. FILTER (regex(str(?outputType), \"https://w3id.org/rains#\" ))} FILTER (regex(str(?elementType), \"https://w3id.org/rains#\" ))}";
	...
	public ArrayList <HashMap>  getPlanElementsForImplementationStageAccountabilityPlan(String systemIri) {
		...
		ArrayList <HashMap> resultSet = new ArrayList <HashMap> ();
			String queryString = Constants.PREFIXES + "Select Distinct ?element ?elementType ?input ?inputType ?output ?outputType ?row FROM <"+getPlansNamedGraph(systemIri)+"> WHERE {?plan rdf:type rains:ImplementationStageAccountabilityPlan; <"+RainsOntologyComponents.specifiedForSystem+"> <"+systemIri+">. ?element ep-plan:isElementOfPlan ?plan.  ?element a ?elementType. OPTIONAL {?element ep-plan:hasInputVariable ?input. ?element af:belongsToRow ?row. ?input a ?inputType. FILTER (regex(str(?inputType), \"https://w3id.org/rains#\" ))} OPTIONAL {?element ep-plan:hasOutputVariable ?output. ?output a ?outputType. FILTER (regex(str(?outputType), \"https://w3id.org/rains#\" ))} FILTER (regex(str(?elementType), \"https://w3id.org/rains#\" ))}";
	...
	public ArrayList <HashMap>  getPlanElementsForDesignStageAccountabilityPlan(String systemIri) {
		...
		ArrayList <HashMap> resultSet = new ArrayList <HashMap> ();
			String queryString = Constants.PREFIXES + "Select Distinct ?element ?elementType ?input ?inputType ?output ?outputType ?row FROM <"+getPlansNamedGraph(systemIri)+"> WHERE {?plan rdf:type rains:DesignStageAccountabilityPlan; <"+RainsOntologyComponents.specifiedForSystem+"> <"+systemIri+">. ?element ep-plan:isElementOfPlan ?plan.  ?element a ?elementType. OPTIONAL {?element ep-plan:hasInputVariable ?input. ?element af:belongsToRow ?row. ?input a ?inputType. FILTER (regex(str(?inputType), \"https://w3id.org/rains#\" ))} OPTIONAL {?element ep-plan:hasOutputVariable ?output. ?output a ?outputType. FILTER (regex(str(?outputType), \"https://w3id.org/rains#\" ))} FILTER (regex(str(?elementType), \"https://w3id.org/rains#\" ))}";
	...
	public ArrayList <HashMap>  getPlanElementsForInstallationAccountabilityPlan(String systemIri) {
		...
		ArrayList <HashMap> resultSet = new ArrayList <HashMap> ();
			String queryString = Constants.PREFIXES + "Select Distinct ?element ?elementType ?input ?inputType ?output ?outputType ?row FROM <"+getPlansNamedGraph(systemIri)+"> WHERE {?plan rdf:type rains:InstallationAccountabilityPlan; <"+RainsOntologyComponents.specifiedForSystem+"> <"+systemIri+">. ?element ep-plan:isElementOfPlan ?plan.  ?element a ?elementType. OPTIONAL {?element ep-plan:hasInputVariable ?input. ?element af:belongsToRow ?row. ?input a ?inputType. FILTER (regex(str(?inputType), \"https://w3id.org/rains#\" ))} OPTIONAL {?element ep-plan:hasOutputVariable ?output. ?output a ?outputType. FILTER (regex(str(?outputType), \"https://w3id.org/rains#\" ))} FILTER (regex(str(?elementType), \"https://w3id.org/rains#\" ))}";
	...
	public ArrayList <HashMap>  getPlanElementsForUserTrainingAccountabilityPlan(String systemIri) {
		...
		ArrayList <HashMap> resultSet = new ArrayList <HashMap> ();
			String queryString = Constants.PREFIXES + "Select Distinct ?element ?elementType ?input ?inputType ?output ?outputType ?row FROM <"+getPlansNamedGraph(systemIri)+"> WHERE {?plan rdf:type rains:UserTrainingAccountabilityPlan; <"+RainsOntologyComponents.specifiedForSystem+"> <"+systemIri+">. ?element ep-plan:isElementOfPlan ?plan.  ?element a ?elementType. OPTIONAL {?element ep-plan:hasInputVariable ?input. ?element af:belongsToRow ?row. ?input a ?inputType. FILTER (regex(str(?inputType), \"https://w3id.org/rains#\" ))} OPTIONAL {?element ep-plan:hasOutputVariable ?output. ?output a ?outputType. FILTER (regex(str(?outputType), \"https://w3id.org/rains#\" ))} FILTER (regex(str(?elementType), \"https://w3id.org/rains#\" ))}";
	...
	public ArrayList <HashMap>  getPlanElementsForManualProductionAccountabilityPlan(String systemIri) {
		...
		ArrayList <HashMap> resultSet = new ArrayList <HashMap> ();
			String queryString = Constants.PREFIXES + "Select Distinct ?element ?elementType ?input ?inputType ?output ?outputType ?row FROM <"+getPlansNamedGraph(systemIri)+"> WHERE {?plan rdf:type rains:ManualProductionAccountabilityPlan; <"+RainsOntologyComponents.specifiedForSystem+"> <"+systemIri+">. ?element ep-plan:isElementOfPlan ?plan.  ?element a ?elementType. OPTIONAL {?element ep-plan:hasInputVariable ?input. ?element af:belongsToRow ?row. ?input a ?inputType. FILTER (regex(str(?inputType), \"https://w3id.org/rains#\" ))} OPTIONAL {?element ep-plan:hasOutputVariable ?output. ?output a ?outputType. FILTER (regex(str(?outputType), \"https://w3id.org/rains#\" ))} FILTER (regex(str(?elementType), \"https://w3id.org/rains#\" ))}";
	...
	public ArrayList <HashMap>  getPlanElementsForDeploymentStageAccountabilityPlan(String systemIri) {
		...
		ArrayList <HashMap> resultSet = new ArrayList <HashMap> ();
			String queryString = Constants.PREFIXES + "Select Distinct ?element ?elementType ?input ?inputType ?output ?outputType ?row FROM <"+getPlansNamedGraph(systemIri)+"> WHERE {?plan rdf:type rains:DeploymentStageAccountabilityPlan; <"+RainsOntologyComponents.specifiedForSystem+"> <"+systemIri+">. ?element ep-plan:isElementOfPlan ?plan.  ?element a ?elementType. OPTIONAL {?element ep-plan:hasInputVariable ?input. ?element af:belongsToRow ?row. ?input a ?inputType. FILTER (regex(str(?inputType), \"https://w3id.org/rains#\" ))} OPTIONAL {?element ep-plan:hasOutputVariable ?output. ?output a ?outputType. FILTER (regex(str(?outputType), \"https://w3id.org/rains#\" ))} FILTER (regex(str(?elementType), \"https://w3id.org/rains#\" ))}";
...
public HashMap <String,HashSet <String >> getStepComponentHierarchy (String plantype) {
	...
	if (plantype.equals("deploymentInstallation")) {
		queryString = Constants.PREFIXES + "Select Distinct ?step ?parentStep FROM <"+Constants.WORKFLOW_COMPONENTS_NAMED_GRAPH_IRI+"> { ?step rdfs:subClassOf* ep-plan:MultiStep. ?step rdfs:subClassOf ?parentStep.?step rdfs:subClassOf ?restriction. ?restriction owl:onProperty ep-plan:isStepOfPlan. { ?restriction owl:allValuesFrom rains:InstallationAccountabilityPlan ;  } UNION { ?restriction owl:allValuesFrom/owl:unionOf/rdf:rest*/rdf:first rains:InstallationAccountabilityPlan.} FILTER (!isBlank(?parentStep))}order  by  asc (?step) ";
		
	}
	
	if (plantype.equals("deploymentUserTraining")) {
		queryString = Constants.PREFIXES + "Select Distinct ?step ?parentStep FROM <"+Constants.WORKFLOW_COMPONENTS_NAMED_GRAPH_IRI+"> { ?step rdfs:subClassOf* ep-plan:MultiStep. ?step rdfs:subClassOf ?parentStep.?step rdfs:subClassOf ?restriction. ?restriction owl:onProperty ep-plan:isStepOfPlan. { ?restriction owl:allValuesFrom rains:UserTrainingAccountabilityPlan ;  } UNION { ?restriction owl:allValuesFrom/owl:unionOf/rdf:rest*/rdf:first rains:UserTrainingAccountabilityPlan.} FILTER (!isBlank(?parentStep))}order  by  asc (?step) ";
		
	}
	
	if (plantype.equals("deploymentManual")) {
		queryString = Constants.PREFIXES + "Select Distinct ?step ?parentStep FROM <"+Constants.WORKFLOW_COMPONENTS_NAMED_GRAPH_IRI+"> { ?step rdfs:subClassOf* ep-plan:MultiStep. ?step rdfs:subClassOf ?parentStep.?step rdfs:subClassOf ?restriction. ?restriction owl:onProperty ep-plan:isStepOfPlan. { ?restriction owl:allValuesFrom rains:ManualProductionAccountabilityPlan ;  } UNION { ?restriction owl:allValuesFrom/owl:unionOf/rdf:rest*/rdf:first rains:ManualProductionAccountabilityPlan.} FILTER (!isBlank(?parentStep))}order  by  asc (?step) ";
...
public HashSet getAgentsInExecutionTraces(String systemIRI) {
		...
		String queryString = Constants.PREFIXES + "Select Distinct ?agent "+fromPart+" Where { ?activity  prov:wasAssociatedWith ?agentEntity. ?agentEntity prov:value ?agent. }";
