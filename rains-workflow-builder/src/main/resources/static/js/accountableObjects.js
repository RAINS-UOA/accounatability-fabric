...
function loadAccountableObjectsInDiv () {
	...
	objetResult.then(
			...
			document.getElementById("MainAddObjectLink").innerHTML = '<li class="typeWidget"> <div class="class="col-md-12 ""><a href="#" data-toggle="modal" data-target="#accountableObjectModal" onclick="addAccountableObjectTo(\''+btoa(document.getElementById("systemIRI").value)+'\')"> Add Object</a> </div></li>';
	...
	if (accountableObjects[parentIR] !=null) {
	 ...
	 //for (let i=0;i<childElementsArray.length;i++) {
		...
		console.log(value);
		 ...
		 html = html + '<li  ><span id='+key+'  class="caret container typeWidget"   >'+  value+'</span>  <ul class="nested container"> <li class="typeWidget"> <a href="#" data-toggle="modal" data-target="#accountableObjectModal" onclick="addAccountableObjectTo(\''+btoa(key)+'\')" > Add Object</a> </li> '+nestedElement+' </ul></li><br>';
